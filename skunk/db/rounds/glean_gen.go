...
const cols = " `id`, `external_id`, `player`, `status`, `submitted`, `created_at`, `updated_at` "
...
func scan(row row) (*skunk.Round, error) {
	...
	err := row.Scan(&g.ID, &g.ExternalID, &g.Player, &g.Status, &g.Submitted, &g.CreatedAt, &g.UpdatedAt)
	if err != nil {
		return nil, err
	}
